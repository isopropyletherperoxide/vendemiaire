...
dateToFrench year month day = (yearToFrench year day', getFrenchMonthFromDay frenchDay, monthDay frenchDay)
  where
    frenchDay = dayToFrench_ year day'
...
dayOfYear year month day
  | isLeap year = foldl' (+) 0 (take (month - 1) leapyear) + day
  | otherwise = foldl' (+) 0 (take (month - 1) normalyear) + day

dayToFrench_ :: Int -> Int -> Int
dayToFrench_ year day
  | isLeap year = dayToFrenchLeap day
...
yearToFrench year civil_day
  | isLeap year && civil_day < 266 = year - 1792
...
getFrenchMonthFromDay a
  | a > 360 = Sansculottides
